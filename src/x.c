...
winprop_t wid_get_prop_adv(const session_t *ps, xcb_window_t w, xcb_atom_t atom,
                           uint32_t offset, uint32_t length, xcb_atom_t rtype, int rformat) {
 ...
 */
xcb_render_picture_t
x_create_picture_with_pictfmt(xcb_connection_t *c, xcb_drawable_t d, uint16_t wid, uint16_t hei,
...
xcb_render_picture_t
x_create_picture_with_visual(xcb_connection_t *c, xcb_drawable_t d, uint16_t w,
                             uint16_t h, xcb_visualid_t visual, uint32_t valuemask,
                             const xcb_render_create_picture_value_list_t *attr) {
...
void x_set_picture_clip_region(xcb_connection_t *c, xcb_render_picture_t pict,
	...
	for (int i = 0; i < nrects; i++)
		xrects[i] = (xcb_rectangle_t){
		    .x = (int16_t)rects[i].x1,
		    .y = (int16_t)rects[i].y1,
		    .width = (uint16_t)(rects[i].x2 - rects[i].x1),
		    .height = (uint16_t)(rects[i].y2 - rects[i].y1),
		};

	xcb_generic_error_t *e = xcb_request_check(
	    c, xcb_render_set_picture_clip_rectangles_checked(
	           c, pict, clip_x_origin, clip_y_origin, (uint32_t)nrects, xrects));
	if (e)
 ...
 */
uint32_t x_picture_filter_from_conv(const conv *kernel, double center,
	...
	return (size_t)(kernel->w * kernel->h + 2);
}
